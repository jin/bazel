  ...
  private class NodeVisitor implements Runnable {
    ...
    public void run() {
      ...
      for (SkyKey rdep : nodeEntry.getReverseDepsForDoneEntry()) {
        ...
        execute(new NodeVisitor(rdep, keptRdeps, keptDeps, verificationSet, verificationSetSeen));
      }
      ...
      for (SkyKey dep : nodeEntry.getDirectDeps()) {
        ...
        execute(new CollectVerificationSet(dep, verificationSet, keptRdeps, verificationSetSeen));
        ...
        if (dep instanceof ArtifactNestedSetKey) {
          for (Artifact a : ((ArtifactNestedSetKey) dep).expandToArtifacts()) {
            ...
            keptDeps.add(aKey);
            execute(
                new CollectVerificationSet(aKey, verificationSet, keptRdeps, verificationSetSeen));
          }
        }
      }
    }
    ...
    private class CollectVerificationSet implements Runnable {
      ...
      @Override
      public void run() {
        if (!verificationSetSeen.add(key)) {
          // This subgraph has been processed, don't do it again.
          return;
        }

        if (keptRdeps.contains(key)) {
          // This node is part of the working set (or its reverse TC), don't proceed.
          return;
        }

        if (key instanceof RootedPath) {
          verificationSet.add((RootedPath) key);
        } else {
          InMemoryNodeEntry nodeEntry = graph.getIfPresent(key);
          ...
          nodeEntry
              ...
              .forEach(
                  dep ->
                      execute(
                          new CollectVerificationSet(
                              dep, verificationSet, keptRdeps, verificationSetSeen)));
        }
      }
    }
  }
  ...
  private FocusResult run(Set<SkyKey> roots, Set<SkyKey> leafs) throws InterruptedException {
    ...
    try (SilentCloseable c = Profiler.instance().profile("focus.mark")) {
      ...
      for (SkyKey leaf : leafs) {
        execute(new NodeVisitor(leaf, keptRdeps, keptDeps, verificationSet, verificationSetSeen));
      }
      ...
      graph.parallelForEach(
          inMemoryNodeEntry -> {
            ...
            if (verificationSet.contains(key)) {
              // We need to keep the nodes in the verification set because the
              // FilesystemValueChecker relies on their existence to check for
